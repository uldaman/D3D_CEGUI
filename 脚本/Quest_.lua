...
function Quest.预备远征(bHave)
    if bHave == true then -- 已有任务, 去做任务
        if MH_是否已完成任务("预备远征") == 1 then -- 判断是否完成
            ...
            MH_调试("MH_做任务")
            --升级到15
            Instance.S_全自动刷本("试练！鬼狩蛛", "身体", 0, 0.9)
						MH_收起武器()
    				MH_延时(10000)
						MH_过滤式采集("尸体")
        end
    ...
    if bHave == true then -- 已有任务, 去做任务
        if MH_是否已完成任务("罗登的见面礼") == 1 then -- 判断是否完成
            ...
            MH_容错式喝药()
		    		...
		    		Instance.S_副本过图(7)
		    		Instance.S_副本过图(0) 
           	while MH_是否已完成任务("罗登的见面礼") ~= 1 and MH_是否工作() == 1  do
		          MH_过滤式采集("可疑的香草")
							Instance.S_副本过图(2)
							Instance.S_副本过图(0) 
							MH_走到目标点(806.89,3170.64)
		    			MH_延时(1000)
		    			MH_交纳物品()
    				end
    				MH_关闭无敌()
        ...
        if MH_是否已完成任务("有仇必报的猎人") == 1 then -- 判断是否完成
            ...
            MH_调试("MH_做任务")
    			...
    			MH_开启无敌()
						Instance.S_副本过图(4) 
						Instance.S_副本过图(0)
          while MH_是否已完成任务("有仇必报的猎人") ~= 1 and MH_是否工作() == 1 do
    ...
    if bHave == true then -- 已有任务, 去做任务
        if MH_是否已完成任务("莉莉的美食单") == 1 then -- 判断是否完成
            ...
            MH_进入副本("[莉莉的美食单]")
	    			MH_延时(20000)
    			MH_开启无敌()
          while MH_是否已完成任务("莉莉的美食单") ~= 1 and MH_是否工作() == 1 do
						Instance.S_副本过图(2)
						Instance.S_副本过图(0)
    ...
    if bHave == true then -- 已有任务, 去做任务
        if MH_是否已完成任务("哨所的奖励") == 1 then -- 判断是否完成
            if MH_是否在副本() == 0 then -- 完成了, 去交任务
              ...
              MH_精准交任务("哨所的奖励", "维奥娜", 1335.36,1238.71)
            end
        else -- 没完成任务去做任务
            MH_调试("MH_做任务")
        end
...
function Quest.年轻的仰慕者(bHave)
    if bHave == true then -- 已有任务, 去做任务
        ...
        if MH_是否在副本() == 0 then
            City.S_地图修正("梅杰波尔坦")
            MH_精准接任务("年轻的仰慕者", "维奥娜", 1335.36,1238.71)
        end
    end
end
...
function Quest.新人地狱(bHave)
    if bHave == true then -- 已有任务, 去做任务
        if MH_是否已完成任务("新人地狱") == 1 then -- 判断是否完成
            ...
            MH_调试("MH_做任务")
            MH_进入副本("[涅尔的新人培训]")--27级时打此副本经验还有331，可以做为升级用副本
    				MH_延时(30000)
    				MH_击杀怪物("蓝速龙", 5)
        end
    else -- 没有任务去接任务
...
function Quest.培育香菇(bHave)
    if bHave == true then -- 已有任务, 去做任务
        if MH_是否已完成任务("培育香菇") == 1 then -- 判断是否完成
            ...
            MH_调试("MH_做任务")
            Instance.S_全自动刷本("试练！河狸兽", "身体", 1, 0.9)
						MH_收起武器()
    				MH_延时(2000)
						MH_过滤式采集("尸体")
        end
...
function Quest.英雄的资格(bHave)
    if bHave == true then -- 已有任务, 去做任务
        if MH_是否已完成任务("英雄的资格") == 1 then -- 判断是否完成
            if MH_是否在副本() == 0 then -- 完成了, 去交任务
            	MH_调试("英雄的资格 MH_交任务")
              City.S_地图修正("耀月林")
              MH_交任务("英雄的资格", "埃兰")
            ...
            Instance.S_副本过图(5)
    				MH_延时(50000)
    				Instance.S_全自动刷本("[英雄的资格]", "身体", 0, 0.9)
						MH_收起武器()
    				MH_延时(10000)
						MH_过滤式采集("尸体")
        end
...
function Quest.主线()
    ...
    while MH_是否工作() == 1 do
        ...
        if MH_获取人物等级()>14 and MH_获取经验上限()<2000 then
